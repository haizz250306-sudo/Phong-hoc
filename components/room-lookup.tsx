      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 rounded-xl border border-white/70 bg-white/70 px-4 py-2.5 shadow-sm backdrop-blur">
          ...
          <div className="leading-none">
            ...
            <span className="font-mono text-lg font-bold tabular-nums text-foreground">{NOW} AM</span>
          </div>
        </div>
        <button
          type="button"
...
function RoomCard({ room, onOpen, onCancel }: { room: Room; onOpen: () => void; onCancel: () => void }) {
        ...
        <div className="space-y-5 p-5">
          <div className="flex gap-3 rounded-xl border border-amber-200 bg-amber-50 p-3.5 text-sm">
            ...
            <p className="text-amber-800">
              Phòng <span className="font-bold">{room.name}</span> đang trống. Lớp học chính khóa tiếp theo bắt đầu lúc{" "}
              <span className="font-bold">{room.nextClass}</span>{" "}
              <span className="text-amber-700">(Bạn còn {formatDuration(gapMinutes)} sử dụng).</span>
            </p>
          </div>
